...
#define CO_OD_FLASH_PARAM_DEFAULT                                              \
    LAST_PAGE_ADDRESS - (2 * PAGES_PER_FLASH_AREA * FLASH_PAGE_SIZE)
...
enum CO_RestoreFunctionality_Flags { RESTORES_PARAMETERS = 0x01 };
 ...
 *                         default parameters
 */
static CO_SDO_abortCode_t storeParameters(uint32_t FlashAddress,
                                          uint8_t ParametersSub)
{
    ...
    for (addressPtr = FlashAddress; addressPtr < addressFinal;
         addressPtr += 4) {
...
static CO_SDO_abortCode_t restoreParameters(uint32_t FlashAddress,
                                            uint8_t ParametersSub)
{
    CO_UNUSED(ParametersSub);
...
static CO_SDO_abortCode_t CO_ODF_1011_RestoreParam(CO_ODF_arg_t *ODF_arg)
{
    ...
    CO_SDO_abortCode_t Result =
        restoreParameters(CO_OD_FLASH_PARAM_DEFAULT, ODF_arg->subIndex);

    if (Result != CO_SDO_AB_NONE) {
        return Result;
    }

    return storeParameters(CO_OD_FLASH_PARAM_RUNTIME,
                           OD_H1011_RESTORE_PARAM_ALL);
}
...
void CO_FlashInit(void)
{
    ...
    flash_read(CO_OD_FLASH_PARAM_DEFAULT,
               &DefaultObjDicParam,
               sizeof(DefaultObjDicParam));
...
{
    CO_OD_configure(*(CO->SDO),
                    OD_H1010_STORE_PARAM_FUNC,
                    CO_ODF_1010_StoreParam,
                    (void *)0,
                    0,
                    0);
