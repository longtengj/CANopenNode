...
const uint16_t FIRST_HB_TIME = 500;
const uint16_t SDO_SRV_TIMEOUT_TIME = 1000;
const uint16_t SDO_CLI_TIMEOUT_TIME = 500;
...
int main(void)
{
    ...
    uint8_t activeNodeId = 10;  /* Copied from CO_pendingNodeId in the
...
#if (CO_CONFIG_STORAGE) & CO_CONFIG_STORAGE_ENABLE
    err = CO_storageFlash_init(&storage,
                               CO->CANmodule,
